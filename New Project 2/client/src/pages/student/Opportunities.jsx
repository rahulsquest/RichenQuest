...
function OpportunityCard({ o }) {
  const reasons = o.why_this_matches || o.match_reasons || o.reasons || [];
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {[o.country, o.type, o.programme].filter(Boolean).join(' · ')}
          </p>
        </div>
        {o.recommendation_rank != null && (
          <span className="shrink-0 rounded-full bg-slate-900 text-white text-xs font-bold px-2.5 py-1">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
        <Field icon={Wallet} label="Tuition" value={o.tuition} />
        <Field icon={Wallet} label="Living cost" value={o.living_cost} />
        <Field icon={CalendarClock} label="Deadline" value={o.deadline} />
        <Field label="Eligibility" value={o.eligibility} />
      ...
      {(o.verified_date || o.source_url) && (
        <p className="text-[11px] text-slate-400 mt-3 flex items-center gap-1.5">
          <ShieldCheck className="w-3 h-3" />
          {o.verified_date ? `Verified ${o.verified_date}` : 'Verified'}
          {o.source_url && <span className="truncate">· {o.source_url}</span>}
        </p>
      )}
    </article>
  );
